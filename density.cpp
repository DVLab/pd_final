...
void ExampleFunction::evaluateFG(const vector<double> &x, double &f, vector<double> &g)
{
   ...
   double D = 0;
	...
	double M = 1.0*(binWidth*binHeight - 0); // assume t_density = 1.0 , area of preplaced block = 0
	//cout<<"M "<<M<<endl;
	...
	for(size_t i = 0 ; i < binCenterX.size() ; i++)
		cout<<binCenterX[i]<<" ";
	cout<<endl;
	for(size_t i = 0 ; i < binCenterY.size() ; i++)
		cout<<binCenterY[i]<<" ";
	cout<<endl;
	for(size_t i = 0 ; i < binCenterZ.size() ; i++)
		cout<<binCenterZ[i]<<" ";
	cout<<endl;
...
void ExampleFunction::evaluateF(const vector<double> &x, double &f)
{
   ...
   double dPx,dPy,dPz; 
   double D = 0;
	double M; // D--density function  M--density target
	...
	for(unsigned y = 0 ; y < sqrt(binNum) ; y++){
		for(unsigned z = 0 ; z < sqrt(binNum) ; z++){
			for(unsigned i = 0 ; i < sqrt(binNum) ; i++){
				...
				M = 1.0*(binWidth*binHeight - 0); // assume t_density = 1.0 , area of preplaced block = 0
				f += (D - M)*(D - M);
			}
		}
	}
